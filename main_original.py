...
def run_selected_accounts():
    ...
    while not STOP_FLAG:
        ...
        delay_minutes = random.randint(120, 360)  # 2-6 jam untuk hindari banned
        delay_seconds = delay_minutes * 60
        delay_hours = delay_minutes / 60

        print(f"{C}Semua akun selesai. Jeda {delay_hours:.1f} jam ({delay_minutes} menit) sebelum cycle berikutnya...{W}")
        ...
        for remaining_minutes in range(delay_minutes, 0, -1):
            ...
            print(f"⏳ Sisa waktu: {remaining_hours:.1f} jam ({remaining_minutes} menit)...", end='\r')
