...
class VKSerfingBot:
    ...
    def _ensure_tg_client(self):
        """Lazy init telegram client when needed - PRIORITAS SESSION FILE (lebih stabil)"""
        ...
        session_string = tg_config.get('session_string')
        session_file = tg_config.get('session_file') or tg_config.get('session')
        ...
        try:
            ...
            # PRIORITAS 1: Session file (lebih stabil, tidak corrupt)
            if session_file:
                try:
                    import os
                    ...
                    if os.path.exists(session_path + '.session'):
                        ...
                        if self.tg_client.is_user_authorized():
                            print(f"{G}✓ TG loaded from session file (stable){W}")
                            return True
            ...
            # PRIORITAS 2: Auto-detect session file by account name (telegram.session atau telegram_*.session)
            if hasattr(self, 'account_name') and self.account_name:
                try:
                    import os
                    accounts_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'accounts')
                    ...
                    if os.path.exists(session_dir):
                        for f in os.listdir(session_dir):
                            if f.endswith('.session') and ('telegram' in f.lower()):
                    ...
                    if session_files:
                        ...
                        self.tg_client.connect()
                        if self.tg_client.is_user_authorized():
                            print(f"{G}✓ TG loaded from auto-detected session (stable){W}")
                            
                            # Update config to use session_file (tidak convert ke string)
                            try:
                                self.config['telegram']['session_file'] = session_file
                            except:
                                pass
            ...
            if session_string and session_string != 'null':
                try:
                    ...
                    self.tg_client.connect()
                    if self.tg_client.is_user_authorized():
                        print(f"{Y}⚠ TG loaded from string session (less stable){W}")
                        return True
