...
try:
    ...
    def get_error_description(code): return f"Error {code}"
    def is_skippable_error(code): return code in [15, 17, 18, 100, 113]
    def is_rate_limit_error(code): return code == 6
...
class VKApi:
    ...
    def set_profile(self, bdate: str = None, city_id: int = None, country_id: int = None, home_town: str = None) -> Dict:
        """
        Save profile info

        Args:
            bdate: Birth date (D.M.YYYY)
            city_id: City ID
            country_id: Country ID
            home_town: Home town name
        ...
        return self._call_method('account.saveProfileInfo', params)
    ...
    def set_bio(self, text: str) -> Dict:
        ...
        Returns:
            API response
        """
        return self._call_method('status.set', {'text': text})
