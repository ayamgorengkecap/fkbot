...
def import_session_file(account_folder, session_file_path):
    """Import existing Telegram session file (.session) ke config - TANPA convert ke string"""
    
    config_path = os.path.join(ACCOUNTS_DIR, account_folder, 'config.json')
    acc_dir = os.path.join(ACCOUNTS_DIR, account_folder)
    ...
    try:
        ...
        # Copy session file ke account folder (TANPA convert ke string)
        dest_session = os.path.join(acc_dir, 'telegram.session')
        if os.path.abspath(session_file_path) != os.path.abspath(dest_session):
            shutil.copy2(session_file_path, dest_session)
            print(f"{C}  → Session file copied to {account_folder}/telegram.session{W}")
        
        # Update config - pakai session_file, BUKAN session_string
        ...
        config['telegram'] = {
            ...
            'phone': me.phone or '',
            'session_file': 'telegram.session',  # Pakai file, bukan string
            'session_string': None,  # Explicitly set None
            'user_id': me.id,
        ...
        print(f"{G}✓ Session file berhasil di-import ke {account_folder}/{W}")
        print(f"{C}  Menggunakan session FILE (lebih stabil dari string){W}")
...
def generate_and_bind_session(account_folder, phone):
    """Generate Telegram session dari phone number dan bind ke config - SAVE KE FILE"""
    ...
    try:
        ...
        # Save langsung ke file di account folder
        session_path = os.path.join(acc_dir, 'telegram')
        ...
        # Update config - pakai session_file
        with open(config_path, 'r') as f:
            config = json.load(f)
        ...
        print(f"{G}✓ Session FILE berhasil disimpan ke {account_folder}/telegram.session{W}")
        print(f"{C}  Menggunakan session FILE (lebih stabil dari string){W}")
        print(f"{C}  Telegram tasks auto-enabled{W}")
